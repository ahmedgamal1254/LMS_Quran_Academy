						...
						<div class="nav nav-item  navbar-nav-right ml-auto">
									...
									<div class="menu-header-content bg-primary text-right">
										<div class="d-flex">
                                            ...
                                            @if (Auth::guard('admin')->user()->unreadNotifications->count() != 0)
                                                <a href="{{ route("offer.read_all") }}" class="badge badge-pill badge-warning mr-auto my-auto float-left">
										...
										<p class="dropdown-title-text subtext mb-0 text-white op-6 pb-0 tx-12 ">
                                           لديك  {{ Auth::guard('admin')->user()->unreadNotifications->count() }} من الاشعارات الغير مقروئة</p>
									</div>
									<div class="main-notification-list Notification-scroll">
                                        @forelse (Auth::guard('admin')->user()->unreadNotifications as $notify)
                                            <a class="d-flex p-3" href="{{ route("offer.show",[$notify->data["offer_id"],$notify->id]) }}">
                                                <div class="notifyimg bg-purple">
                                                    <i class="la la-gem text-white"></i>
                                                </div>
                                                <div class="mr-3">
                                                    <h5 class="notification-label mb-1">تم اضافة العرض هذا {{ $notify->data["offer_name"] }}</h5>
                                                    <h5 class="notification-label mb-1">بواسطة :-  {{ $notify->data["name"] }}</h5>
							...
							<div class="dropdown main-profile-menu nav nav-item nav-link">
								...
								<div class="dropdown-menu">
									...
									<form class="dropdown-item" action="{{ route("logout_admin") }}" method="post">
										@csrf
										<button style="border: none;background: transparent;" type="submit"><i class="bx bx-log-out"></i>Sign Out</button>
									</form>
								</div>
							</div>
						</div>
